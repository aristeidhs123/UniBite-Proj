...
function initFeed() {
    ...
    if (allergenFilter) {
        ...
        allergenFilter.addEventListener("change", () => {
            selectedAllergen = allergenFilter.value;
            loadListings();
        });
    }
    ...
    if (ingredientFilter) {
        ...
        ingredientFilter.addEventListener("change", () => {
            selectedIngredient = ingredientFilter.value;
            loadListings();
        });
    }
...
function loadListings() {
    ...
    if (selectedAllergen) {
        visibleMeals = visibleMeals.filter(meal => Array.isArray(meal.allergens) && meal.allergens.some(a => String(a).toLowerCase() === selectedAllergen.toLowerCase()));
    }
    ...
    if (selectedIngredient) {
        visibleMeals = visibleMeals.filter(meal => {
            const title = String(meal.title || "").toLowerCase();
            const desc = String(meal.desc || "").toLowerCase();
            return title.includes(selectedIngredient) || desc.includes(selectedIngredient);
        });
    }
...
function renderAllergenOptions() {
    const allergenFilter = document.getElementById("allergen-filter");
    if (!allergenFilter) return;

    const allergens = getAllAllergens();
    allergenFilter.innerHTML = `<option value="">All Allergens</option>` +
        allergens.map(allergen => `<option value="${escapeHtml(allergen)}">${escapeHtml(allergen)}</option>`).join("");
}
...
function renderIngredientOptions() {
    const ingredientFilter = document.getElementById("ingredient-filter");
    if (!ingredientFilter) return;

    const ingredients = getAllIngredients();
    ingredientFilter.innerHTML = `<option value="">All Ingredients / Cuisines</option>` +
        ingredients.map(ingredient => `<option value="${escapeHtml(ingredient)}">${escapeHtml(ingredient)}</option>`).join("");
}
