...
function seedDemoMeals() {
    const existingMeals = mealService.getAll();
...
function seedDemoUsers() {
    ...
    const normalizedExisting = existingUsers.map(u => normalizeUser(u.username));
    demoUsers.forEach(user => {
        if (!normalizedExisting.includes(normalizeUser(user.username))) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof seedDemoUsers === "function") {
        seedDemoUsers();
    }
